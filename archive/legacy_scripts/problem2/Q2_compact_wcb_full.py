# -*- coding: utf-8 -*-
"""
Created on Tue Sep 23 14:11:09 2025

@author: JZX
"""

# Q2_bestfit_wcb.py  第二问：Best-Fit + WCB 紧凑地址 + 动态拓扑
...
#         self.cap = cap
#         self.free = [(0, cap)]          # (start,len) 按地址排序
#         self.alloc = {}                 # buf→(start,size,copy_in?)
#         self._spill_log = []
#         self._total_cost = 0
...
class CompactPool:
    def __init__(self, cap):
        self.cap = cap
        self._free_blocks = [(0, cap)]      # 改名，避免和 free() 冲突
        self.alloc_map = {}                 # buf→(start,size,copy_in?)
        self._spill_log = []
        self._total_cost = 0
    ...
    @property
    def addr_offset(self):
        return {b: st for b, (st, _, _) in self.alloc_map.items() if st != -1}

    # ---- 对外 ----
    # def alloc(self, nid, size, copy_in, cur_idx, future_func, nodes, w1=1.0, w2=1.0):
    # #def alloc(self, nid, size, copy_in, cur_idx, future_func, nodes, w1=1.0, w2=1.0):
    #     pool_type = nodes.loc[nid].Type
    #     print(f"[ALLOC] nid={nid}, type={pool_type}, size={size}, total_free={self._total_free()}")
    ...
    def alloc(self, nid, size, copy_in, cur_idx, future_func, nodes, w1=1.0, w2=1.0):
        ...
        # 2. 洞尺寸二次保障：预算够了但最大洞仍 < size 就继续踢
        while self._best_fit(size) is None and self._total_free() >= size:
            extra = self._choose_wcb(1, cur_idx, future_func, w1, w2)  # 至少踢 1B
            victims.extend(extra)
            self._spill(extra, nodes)
    ...
    def free(self, nid):
        if nid not in self.alloc_map or self.alloc_map[nid][0] == -1:
            return
        st, sz, _ = self.alloc_map[nid]
        self._coalesce(st, sz)   # 内部合并逻辑
    ...
    def _split(self, idx, start, blk, need, nid):
        self.alloc_map[nid] = (start, need, self.alloc_map[nid][2])
        rem = blk - need
        if rem > 0:
    ...
    def _spill(self, victims, nodes):
        ...
        for b in victims:
            ...
            self._total_cost += cost
            self._spill_log.append({"victim": b, "size": sz, "copy_in": ci, "cost": cost})
            self.alloc_map[b] = (-1, sz, ci)  # 标记在 DDR
        return c
...
def simulate_with_spill(nodes, edges, initial_order, w1=1.0, w2=1.0):
    ...
    for idx, nid in enumerate(initial_order):
        ...
        for p in edges[edges["EndNodeId"] == nid]["StartNodeId"].tolist():
            ...
            if st == -1:
                p_pool.alloc(p, sz, ci, idx, future_fun, nodes, w1, w2)
...
def simulate_with_spill(nodes, edges, initial_order, w1=1.0, w2=1.0):
    ...
    # 前驱 SPILL_IN 检查
        ...
        if op == "ALLOC":
            pool = mm._pool_of(nid, nodes)
            #cost, victims = pool.alloc(nid, size, is_ci, idx, future_fun, nodes, w1, w2)
            cost, victims = pool.alloc(nid, size, is_ci, idx, future_fun, nodes=nodes, w1=w1, w2=w2)
            for v in victims:
                out_order.append(f"SPILL_OUT_{v}")
            out_order.append(nid)
        elif op == "FREE":
