...
#             # 内存管理模拟（问题二）
#             # buf_free_positions = {row.BufId: i for i, nid in enumerate(schedule) if nodes.loc[nid].Op == 'FREE'}
#             buf_free_positions = {}
...
#         # 输出纯数值对比
#         print(f"左滑耗时: {slide_time:.3f}s")
#         print(f"原总周期: {T_total}")
#         print(f"左滑后周期: {T_new}")
#         print(f"时间下降: {(T_total - T_new) / T_total * 100:.2f}%")
...
#                 r = nodes.loc[nid]
#                 if r.Op == 'FREE':
#                     buf_free_positions[r.BufId] = i

#             manager = MultiPoolManager(capacities, {})
#             for i, nid in enumerate(schedule):
#                 r = nodes.loc[nid]
#                 if r.Op == 'ALLOC':
#                     free_time = buf_free_positions.get(r.BufId, i+1000)
#                     manager.alloc(r.BufId, r.Size, r.Type, i, free_time)
#                 elif r.Op == 'FREE':
#                     manager.free(r.BufId, r.Type)

#             total_cost, spill_log = manager.collect_spill_info()
#             print(f"搬运量: {total_cost}, SPILL次数: {len(spill_log)}")

#             # 保存 SPILL 文件
#             spill_df = pd.DataFrame(spill_log, columns=["BufId","SpillCost","Time"])
#             spill_df.to_csv(os.path.join(out_dir, f"{case}_spill.csv"), index=False)

#             # === 时钟计算（第三问） ===
#             S, E, T_total = compute_clock(schedule, nodes, edges)
#             S_new, E_new, T_new = conservative_left_slide(schedule, nodes, edges)

#             print(f"流水总时钟 (原): {T_total}")
#             print(f"流水总时钟 (左滑后): {T_new}")
#             if T_total>0:
#                 print(f"时间下降: {(T_total-T_new)/T_total*100:.2f}%")
...
#                 "NodeId": schedule,
#                 "Op": [nodes.loc[nid].Op for nid in schedule],
#                 "Type": [nodes.loc[nid].Type for nid in schedule],
#                 "BufId": [nodes.loc[nid].BufId for nid in schedule],
#                 "Cycles": [nodes.loc[nid].Cycles for nid in schedule],
#                 "Start": [S_new.get(nid, -1) for nid in schedule],
#                 "End": [E_new.get(nid, -1) for nid in schedule]
#             })
#             schedule_df.to_csv(os.path.join(out_dir, f"{case}_schedule.csv"), index=False)
...
#             # 保存 SPILL 详情
#             spill_df = pd.DataFrame(spill_log, columns=["BufId","SpillCost","Time"])
#             spill_df.to_csv(f"{case}_spill.csv", index=False)
...
#                 "End": [E_new.get(nid, -1) for nid in schedule]
#             })
#             schedule_df.to_csv(f"{case}_schedule.csv", index=False)
